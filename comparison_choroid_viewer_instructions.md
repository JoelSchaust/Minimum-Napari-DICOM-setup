...
```bash
dir -> should list everything in the active directory - including the choroid_viewer_seg_comparison.py file 
```
start the viewer.

```bash
python choroid_viewer_seg_comparison.py
```
...
> *this works different to the previous version!!*

| Key | Function |
|-----|----------|
| c   | Mark choroid as **GOOD (1)** |
| v   | Mark vessel as **GOOD (1)** |
| x   | Mark choroid as **BAD (0)** |
| b   | Mark vessel as **BAD (0)** |

The current rating status is displayed in the napari status bar.
Rating an image will not automatically switch to the next image - this is a change because there is more than one input required for each FOV now!
...
The output file is batch dependend, so make sure you set the right paths (default momentarly set to batch 2026-02-16/2026-03-04)